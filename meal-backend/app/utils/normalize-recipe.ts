import type { Recipe, RecipeIngredient, RecipeMacros, RecipeSubstitution } from '@/app/interface';
...
const toMacros = (value: unknown): RecipeMacros => {
  const macros = (value && typeof value === 'object' ? value : {}) as Partial<RecipeMacros>;
...
const toIngredients = (value: unknown): RecipeIngredient[] => {
  ...
  return value.map((item) => {
    const ingredient = (item && typeof item === 'object' ? item : {}) as Partial<RecipeIngredient>;
...
const toSubstitutions = (value: unknown): RecipeSubstitution[] => {
  ...
  return value.map((item) => {
    const substitution = (item && typeof item === 'object' ? item : {}) as Partial<RecipeSubstitution>;

    return {
      ingredient: typeof substitution.ingredient === 'string' ? substitution.ingredient : '',
...
export const normalizeRecipe = (input: unknown): Recipe => {
  const recipe = (input && typeof input === 'object' ? input : {}) as Partial<Recipe>;
  ...
  return {
    ...
    difficulty: recipe.difficulty === 'medium' || recipe.difficulty === 'hard' ? recipe.difficulty : 'easy',
